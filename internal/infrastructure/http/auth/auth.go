...
var (
	identityKey       = "id"
	newJWTMiddleware  = gjwt.New
	initJWTMiddleware = func(m *gjwt.GinJWTMiddleware) error {
		return m.MiddlewareInit()
	}
)
...
func (a *AuthManager) Init() (*gjwt.GinJWTMiddleware, error) {
	if err := a.validateAuthTokenSecret(); err != nil {
		return nil, err
	}

	// the jwt middleware
	authMiddleware, err := newJWTMiddleware(a.initParams())
	if err != nil {
		return nil, fmt.Errorf("JWT Error: %w", err)
	}

	errInit := initJWTMiddleware(authMiddleware)
	if errInit != nil {
...
func (a *AuthManager) validateAuthTokenSecret() error {
	if a.Config == nil {
		return fmt.Errorf("auth config is not set")
	}

	if len(a.Config.AuthTokenSecret) < minAuthTokenSecretLength {
		return fmt.Errorf("AUTH_TOKEN_SECRET must be at least %d characters long", minAuthTokenSecretLength)
	}
...
func (a *AuthManager) identityHandler() func(c *gin.Context) any {
	return func(c *gin.Context) any {
		...
		idClaimRaw, ok := claims[identityKey]
		if !ok {
			return nil
		}

		idClaim, ok := idClaimRaw.(string)
		if !ok || idClaim == "" {
			return nil
		}

		userID, err := uuid.Parse(idClaim)
		if err != nil {
			return nil
		}
